...
2. **Proje planı ve yapılabilirlik** — İş paketleri, süre, kaynak, ekip yetkinliği ve risk yönetimi birbirini tutuyor mu? İddialı hedefe karşılık gevşek bir takvim veya tanımsız başarı kriterleri hakemde güvensizlik yaratır. Bütçe kalemlerini satır satır incelemek gerekiyorsa `gider-kalemi-kontrolu` skill'ini ayrıca çalıştırmayı öner. Taslağın kendi içinde bir sayısal çelişki varsa (örn. bir bölüm "18 ay" derken bütçe gerekçesi 24 aylık hesaba dayanıyorsa) bunu burada bir zayıflık olarak işaretle; kullanıcı ayrıca başka proje belgeleriyle (bütçe dosyası, iş paketleri tablosu vb.) karşılaştırmak isterse `proje-tutarlilik-kontrolu` skill'ini öner.
